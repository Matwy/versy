...
SQUARE_SIZE = 9.0  # Dimensione del quadrato in mm
...
def capture_calibration_images():
    ...
    while img_counter < NUM_IMAGES:
        ...
        if ret_chess:
            ...
            cv2.putText(display_frame, "SCACCHIERA RILEVATA - Dai movate premi SPAZIO", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        else:
            cv2.putText(display_frame, "Scacchiera non rilevata", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        
        # Mostra contatore
        cv2.putText(display_frame, f"Immagini: {img_counter}/{NUM_IMAGES}", 
...
def calibrate_camera():
    ...
    objp *= SQUARE_SIZE  # Scala in base alla dimensione reale
    ...
    # Salva i parametri
    np.savez('config/camera_calibration.npz', 
